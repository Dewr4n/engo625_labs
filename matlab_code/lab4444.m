...
for k=1:9
    ...
    Cl(9+k,9+k)=0.01*0.01;
    %/sin(elevation(k,1)*pi/180)/sin(elevation(k,1)*pi/180)+0.01*0.01/sin(pivox_elevation*pi/180)/sin(pivox_elevation*pi/180) +...
        %0.01*0.01/sin(elevation(k,1)*pi/180)/sin(elevation(k,1)*pi/180)+0.01*0.01/sin(pivox_elevation*pi/180)/sin(pivox_elevation*pi/180);
end
Cx=eye((9+3),(9+3));%%
...
for i=1:epochs_num
    ...
    if ~flag
       ...
       Cx(1:3,1:3)=100*eye(3,3);
       Cx(4:12,4:12)=1e10*eye(9,9);
       % xhat(4:12,1)=zeros(9,1);
    elseif flag
        [A,l]=GetA(xhat(1:3,1), xhat(4:12,1), XB, XS,pr_R,ph_R,pr_B,ph_B,pivox_index,lambda);

        % Cx(1:3,1:3)=1*eye(3,3);
        % Cx(4:12,4:12)=10*eye(9,9);

        m=A*Cx*transpose(A);
        K=Cx*transpose(A)*inv(m+Cl);
        xhat=xhat+K*l;
        Cx=Cx-K*A*Cx;
        Ns(i,:)=xhat(4:12,1)';
    end
    ...
    [e,n,u]=ecef2enu(xhat(1,1),xhat(2,1),xhat(3,1),XB_lla(1),XB_lla(2),XB_lla(3),wgs84);
...
t=t';
subplot(2,2,1)
p=plot(t,pose_errors(2:delta:epochs_num,1),t,sds(2:delta:epochs_num,1),t,-sds(2:delta:epochs_num,1));
p(1).Color='red';
p(2).Color='blue';
p(3).Color='blue';
grid on
...
subplot(2,2,2)
p=plot(t,pose_errors(2:delta:epochs_num,2),t,sds(2:delta:epochs_num,2),t,-sds(2:delta:epochs_num,2));
p(1).Color='red';
p(2).Color='blue';
p(3).Color='blue';
grid on
...
subplot(2,2,3)
p=plot(t,pose_errors(2:delta:epochs_num,3),t,sds(2:delta:epochs_num,3),t,-sds(2:delta:epochs_num,3));
p(1).Color='red';
p(2).Color='blue';
p(3).Color='blue';
grid on
