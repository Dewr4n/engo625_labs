...
def _search_integer_ambiguities(
    ...
    if float_ambiguities.size == 0:
        return np.array([]), np.array([]), float("inf"), float("inf")
    ...
    second_candidate: np.ndarray | None = None
    ...
    if second_candidate is None:
        second_candidate = nearest

    return best_candidate.astype(int), second_candidate.astype(int), best_cost, second_cost
...
def run_dd_positioning(
    ...
    fixed_annotation: np.ndarray | None = None
    ratio_threshold = 1.4
    ...
    for epoch in common_epochs:
        ...
        if converged and not fixed_applied:
            (
                ...
                best_cost,
                second_cost,
            ) = _search_integer_ambiguities(baseline_state[3:], covariance, search_radius=1)
            ratio = second_cost / best_cost if np.isfinite(best_cost) else 0.0
            print(
                f"Epoch {epoch:.1f} ambiguity search: float={baseline_state[3:]}, "
                f"fixed={fixed_ambiguities}, second={second_candidate}, "
                f"best_cost={best_cost:.3f}, second_cost={second_cost:.3f}, ratio={ratio:.2f}"
            )

            if ratio >= ratio_threshold and np.isfinite(best_cost):
                fixed_applied = True
                fixed_epoch = epoch
                fixed_annotation = fixed_ambiguities.copy()
        ...
        fixed_values = (
            fixed_annotation.copy() if fixed_epoch is not None and epoch == fixed_epoch else None
        )
        ...
        results.append(
            DDEpochResult(
                ...
                fixed_ambiguities=fixed_values,
            )
        )
    ...
    if results:
        ...
        rmse_scope = "full interval"
        if fixed_epoch is not None:
            fixed_subset = [res for res in results if res.epoch >= fixed_epoch]
            if fixed_subset:
                eval_results = fixed_subset
                rmse_scope = f"fixed epochs (>= {fixed_epoch:.1f} s)"

        enu_errors = []
        for res in eval_results:
            diff_xyz = res.position - true_xyz
            enu_errors.append(enu_R @ diff_xyz)
        enu_errors = np.vstack(enu_errors)
        axis_labels = ["East", "North", "Up"]
        axis_rmse = np.sqrt(np.mean(enu_errors**2, axis=0))
        three_d_rmse = float(np.sqrt(np.mean(np.sum(enu_errors**2, axis=1))))

        print(f"\n=== ENU RMSE over {rmse_scope} ===")
        for label, rmse_val in zip(axis_labels, axis_rmse):
            print(f"{label} RMSE: {rmse_val:.3f} m")
        print(f"3D RMSE: {three_d_rmse:.3f} m")

        plot_dd_enu(eval_results, true_xyz, enu_R)
