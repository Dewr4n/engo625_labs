...
def plot_dd_ambiguities(results, non_pivot_prns):
    ...
    and annotate fixed ambiguities if they converge to an integer."""
    if not results:
        return
    ...
    for idx, prn in enumerate(non_pivot_prns):
        paired = [(res.epoch - t0, res.ambiguities[idx])
                  for res in results if idx < len(res.ambiguities)]
        if not paired:
            continue

        times, amb_values = zip(*paired)
        amb_values = np.array(amb_values)

        # 原有：浮点模糊度曲线
        plt.plot(times, amb_values, label=f"PRN {prn}")

        # ===== 新增：检测并标注固定模糊度 =====
        # 取最后若干历元的平均作为“固定后”的代表值
        tail = amb_values[-min(10, len(amb_values)):]
        mean_amb = np.mean(tail)
        fixed_int = np.round(mean_amb)

        # 如果已经非常接近整数（比如 < 0.05 周），认为已固定
        if abs(mean_amb - fixed_int) < 0.05:
            plt.hlines(
                fixed_int,
                xmin=times[0],
                xmax=times[-1],
