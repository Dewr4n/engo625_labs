...
CODE_NOISE_DD = 1.5  # meters, conservative for pseudorange double difference
PHASE_NOISE_DD = 0.005  # meters, L1 phase double difference

# Process noise (variance) applied each epoch to keep ambiguities observable in RLS
AMB_PROCESS_NOISE = 0.0001  # (m^2) small random walk to prevent covariance collapse
POS_PROCESS_NOISE = 0.0  # static scenario
...
def _sequential_update(
    ...
    """Recursive least-squares update for linearized system."""

    S = A @ cov @ A.T + R
    K = cov @ A.T @ np.linalg.inv(S)
    delta = K @ w
    state_upd = state + delta

    I = np.eye(len(state))
    cov_upd = (I - K @ A) @ cov @ (I - K @ A).T + K @ R @ K.T
    return state_upd, cov_upd, delta
...
def run_DD_positioning(
    ...
    cov[:3, :3] *= 25.0**2  # 25 m std on initial baseline guess
    cov[3:, 3:] *= 100.0**2  # large uncertainty on ambiguities
    ...
    for t in dd_epochs:
        ...
        # Simple process noise to prevent covariance collapse on ambiguities
        Q = np.zeros_like(cov)
        Q[:3, :3] = POS_PROCESS_NOISE * np.eye(3)
        Q[3:, 3:] = AMB_PROCESS_NOISE * np.eye(len(prns) - 1)
        cov_prior = cov + Q

        rover_xyz = base_xyz + state[:3]
        # Two Gauss-Newton iterations to refine linearization each epoch
        for _ in range(2):
            ...
            # Elevation-dependent measurement weighting so covariance reflects changing geometry
            elev_ref = satellite_elevation(rover_xyz, rover_epoch[ref_prn]["pos"], rover_lat, rover_lon)
            mapping = lambda el: 1.0 / max(np.sin(np.radians(max(el, 5.0))), 1e-3)  # avoid singularities
            ...
            for prn in prns:
                ...
                elev_prn = satellite_elevation(rover_xyz, rover_epoch[prn]["pos"], rover_lat, rover_lon)
                factor = np.sqrt(mapping(elev_ref) ** 2 + mapping(elev_prn) ** 2)
                code_sigmas.append(CODE_NOISE_DD * factor)
                phase_sigmas.append(PHASE_NOISE_DD * factor)

            sigmas = np.concatenate([np.array(code_sigmas), np.array(phase_sigmas)])
            R_meas = np.diag(sigmas**2)

            state, cov, delta = _sequential_update(A, w, state, cov_prior, R_meas)
            cov_prior = cov  # updated covariance used for next inner iteration
            rover_xyz = base_xyz + state[:3]
        ...
        enu_err = R @ (rover_xyz - true_rover_xyz)
        print(
